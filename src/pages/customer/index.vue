    ...
    >
      ...
      >
        <template slot-scope="scope">
          {{ new Date(scope.row.registerTime).getFullYear() +'/'+ (new Date(scope.row.registerTime).getMonth()+1) +'/'+ new Date(scope.row.registerTime).getDate() }}
        </template>
      </el-table-column>
      <el-table-column
        prop="telephone"
...
export default {
  ...
  methods: {
    ...mapActions('customer', ['queryCustomer', 'findAll', 'saveOrUpdate', 'deleteById', 'batchDelete']),
    changePage(page) {
