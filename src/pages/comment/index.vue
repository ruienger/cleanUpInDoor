    ...
    >
      ...
      >
        <template slot-scope="scope">
          <router-link :to="{ name: 'customer_detail', params: {id:scope.row.cusId} }">{{ scope.row.cusId }}</router-link>
        </template>
      </el-table-column>
      <el-table-column
        prop="orderId"
      ...
      >
        <template slot-scope="scope">
          {{ new Date(scope.row.commentTime).getFullYear() +'/'+ (new Date(scope.row.commentTime).getMonth()+1) +'/'+ new Date(scope.row.commentTime).getDate() }}
        </template>
      </el-table-column>
      <el-table-column
        prop="status"
...
export default {
  ...
  methods: {
    ...mapActions('comment', ['queryCustomer', 'findAll', 'saveOrUpdate', 'deleteById', 'toDelHandler']),
    changePage(page) {
