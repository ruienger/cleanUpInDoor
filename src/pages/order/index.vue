    ...
    >
      ...
      >
        <template slot-scope="scope">
          {{ new Date(scope.row.orderTime).getFullYear() +'/'+ (new Date(scope.row.orderTime).getMonth()+1) +'/'+ new Date(scope.row.orderTime).getDate() }}
        </template>
      </el-table-column>
      <el-table-column
        prop="status"
...
export default {
  ...
  methods: {
    ...mapActions('order', ['queryOrder', 'findAll', 'saveOrUpdate', 'deleteById', 'toDelHandler', 'findByCustomerId']),
    ...
    submitForm() {
      ...
      this.form.orderTime = new Date().getTime()
