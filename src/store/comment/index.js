...
export default {
  ...
  actions: {
    ...
    queryCustomer({ commit }, data) {
      axios.post('/comment/query', data).then((result) => {
    ...
    saveOrUpdate({ dispatch }, data) {
      axios.post('/comment/saveOrUpdate', data.form).then((result) => {
    ...
    findCustomerById({ commit }, id) {
      axios.get('/comment/findCustomerById?id=' + id).then((result) => {
